...
def  load_image_gt(dataset, config, image_id, augment=False,
    ...
    def elastic_transform_image(image1, image2, alpha, sigma):
    ...
    if augment:
        ...
        if np.random.random() < 0.3:
            muti_noise = np.random.normal(1, 0.01, (image.shape[0], image.shape[1], 1))
            image *= np.concatenate((muti_noise, muti_noise, muti_noise), axis=2)
        if np.random.random() < 0.5:
            add_noise = np.random.normal(0, 0.1, (image.shape[0], image.shape[1], 1))
            image += np.concatenate((add_noise, add_noise, add_noise), axis=2)
        ...
        # if np.random.random() < 0.5:
        #     image, mask = elastic_transform_image(image, mask, 2000, 50)
